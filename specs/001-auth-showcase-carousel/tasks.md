...
- [ ] T001 Create the asset directory `src/assets/showcase/`
- [ ] T002 Harvest the remaining Figma asset URLs by calling `get_design_context` on nodes `249:7903`
...
- [ ] T003 [P] Export the three mockup images via `get_screenshot` at `maxDimension: 1472` (2× of the
      736 px frame) and save as `src/assets/showcase/website-mockup.png`, `portfolio-mockup.png`,
      `brochure-mockup.png` (depends on T001)
- [ ] T004 [P] Download the three panel textures from the URLs collected in T002 and save as
      `src/assets/showcase/website-texture.png`, `portfolio-texture.png`, `brochure-texture.png`
      (depends on T001, T002)
- [ ] T005 Verify every file in `src/assets/showcase/` is a non-empty PNG and that all six are present,
...
- [ ] T006 Create `src/app/pages/Login/showcase-slides.ts` defining the `ShowcaseStat` and
...
- [ ] T007 Create `src/app/pages/Login/showcase-slides.test.ts` asserting: exactly three slides in
...
- [ ] T008 [P] [US1] Create `src/app/pages/Login/useAutoAdvance.test.ts` with fake timers, asserting:
      ...
      teardown (FR-014)
- [ ] T009 [P] [US1] Extend `src/app/pages/Login/LoginPage.test.tsx` to assert the existing sign-in
...
- [ ] T010 [US1] Create `src/app/pages/Login/useAutoAdvance.ts` implementing the contract in
...
- [ ] T011 [P] [US1] Create `src/app/pages/Login/ShowcaseSlideView.tsx` rendering one slide: texture
...
- [ ] T012 [P] [US1] Create `src/app/pages/Login/SignInPanel.tsx` — the left column, carrying over
...
- [ ] T013 [US1] Create `src/app/pages/Login/ShowcaseCarousel.tsx` wrapping `Carousel`,
...
- [ ] T014 [US1] Rewrite `src/app/pages/Login/LoginPage.tsx` as a full-bleed two-column shell —
      ...
      on T012, T013)
- [ ] T015 [US1] Run `npx vitest run src/app/pages/Login` and confirm T008 and T009 now pass
...
- [ ] T016 [P] [US2] Create `src/app/pages/Login/ShowcaseIndicators.test.tsx` asserting the cumulative
...
- [ ] T017 [US2] Create `src/app/pages/Login/ShowcaseIndicators.tsx` per the contract — 8 px dots
...
- [ ] T018 [US2] Wire `ShowcaseIndicators` into `ShowcaseCarousel`: pass `count`, `selectedIndex` and
...
- [ ] T019 [US2] Verify by keyboard that focus reaches the sign-in button before the dots, and that
...
- [ ] T020 [P] [US3] Extend `src/app/pages/Login/useAutoAdvance.test.ts` to assert no timer starts when
...
- [ ] T021 [US3] Add the reduced-motion branch to `src/app/pages/Login/useAutoAdvance.ts` —
...
- [ ] T022 [US3] Hide the showcase column below the `lg` breakpoint in
...
- [ ] T023 [US3] Resize slowly from 360 px up past the breakpoint **without reloading** and confirm the
...
- [ ] T024 Run the four gates — `npm test && npm run typecheck && npm run build && npm audit` — and
      confirm all pass, with the pre-existing 80 tests still green and no new dependency in the audit
- [ ] T025 [P] Confirm no `as any` was introduced and no Tailwind utility name appears in any prose
...
- [ ] T029 Update `docs/frontend-review.md` to note that `components/ui/carousel.tsx` is no longer
      dormant, since that file records the dormant-component state
- [ ] T030 Update the dormant-component note in `AGENTS.md` for the same reason, keeping it in step with
